...
// export default function UploadImage({ setValue, index, resetValue }) {
...
const UploadImage = forwardRef(({ setValue, index }, ref) => {
  const [logoPreview, setLogoPreview] = useState(null);
  const [upload, { isLoading, isSuccess, data, reset }] = useUploadMutation();
  ...
  }, [isSuccess, isLoading, data, index]);
  ...
  // Expose reset functionality to parent
  useImperativeHandle(ref, () => ({
    resetImage: handleRemoveLogo,
  }));
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      >
        ...
        {isLoading && <Lottie path="/opener-loading.json" loop autoplay />}
        {isSuccess && <Lottie path="/success.json" loop={isLoading} autoplay />}
      </label>
      <input
        id={`logo-input-${index}`}
        type="file"
